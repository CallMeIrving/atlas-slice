...
import { workspace, type VideoFrame, persistMediaSettings, frameImageUrl } from '@/store/workspace'
...
function download(): void { const frame = workspace.video.frames.find((item) => item.id === previewId.value); if (!frame) return; const link = document.createElement('a'); link.href = frameImageUrl(frame); link.download = `frame-${Math.round(frame.timestamp * 1000)}.png`; link.click() }
async function exportSelected(): Promise<void> {
  const selected = workspace.video.frames.filter((frame) => frame.selected)
  await downloadZip(await Promise.all(selected.map(async (frame, index) => ({ name: `frame-${String(index + 1).padStart(4, '0')}.png`, blob: await (await fetch(frameImageUrl(frame))).blob() }))), `${workspace.video.fileName.replace(/\.[^.]+$/, '') || 'video'}-frames.zip`)
}
